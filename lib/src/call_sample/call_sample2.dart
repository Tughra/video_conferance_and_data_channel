...
class PeerToPeerRoom extends StatefulWidget {
  ...
  PeerToPeerRoom({Key? key, required this.ip, required this.type, required this.id})
      : super(key: key);

  @override
  _PeerToPeerRoomState createState() =>
      _PeerToPeerRoomState(serverIP: ip, serverType: type, streamId: id);
}

class _PeerToPeerRoomState extends State<PeerToPeerRoom> {
        ...
        switch (state) {
          ...
          case SignalingState.CallStateBye:
            this.setState(() {
              ...
              _inCalling = false;
              if(Get.currentRoute!="/PeerToPeerRoom")Navigator.pop(context);
            });
            break;
  ...
  _hangUp() {
    if (_signaling != null) {
      _signaling?.closePeer2Peer(streamId);
    }
  }
          ...
          child: Stack(children: <Widget>[
            ...
            Positioned(
                    ...
                    child: Container(    // useful when in a p2p call. small display for self video.
                      width: orientation == Orientation.portrait ? 150.0 : 120.0,
                      height:
                          orientation == Orientation.portrait ? 200.0 : 90.0,
